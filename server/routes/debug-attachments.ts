  ...
  try {
    ...
    const results = {
      ...
      scan: {
        ...
        orphanedFiles: 0,
        missingFiles: 0,
        directorySummary: [] as any[],
      },
      orphanedFiles: [] as any[],
      missingFiles: [] as any[]
    };
    ...
    for (const dir of uploadDirs) {
      try {
        ...
        results.scan.directorySummary.push({
          directory: dir,
          error: `Konnte nicht gescannt werden: ${error instanceof Error ? error.message : 'Unbekannter Fehler'}`,
          filesCount: 0,
          orphanedFilesCount: 0,
          accessible: false
        });
...
router.post('/scan', requireAdmin, fileUpload.single('attachment'), (req: Request, res: Response) => {
  try {
    if (!req.file) {

...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/docs/benutzerhandbuch/download', (req, res) => {
    ...
    // Prüfe, ob ein bestimmtes Format angefordert wurde
    const format = req.query.format?.toString().toLowerCase() || 'md';
    ...
    if (format === 'md') {
      fs.readFile(mdFilePath, 'utf8', (err, data) => {
        ...
        // Hinweis hinzufügen
        const modifiedData = `# Bau-Structura Benutzerhandbuch
> Dieses Dokument ist auch in anderen Formaten verfügbar: [DOCX](/api/docs/benutzerhandbuch/download?format=docx) oder [ePub](/api/docs/benutzerhandbuch/download?format=epub)
...
${data}`;
    ...
    if (format === 'docx') {
      try {
        const { convertMarkdownToDocx } = require('./services/document-converter');
        
        // Asynchroner Prozess für die Konvertierung
        (async () => {
          try {
            // Konvertiere zu DOCX
            const docxPath = await convertMarkdownToDocx(mdFilePath);
            
            // Sende die Datei
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
            res.setHeader('Content-Disposition', 'attachment; filename="Bau-Structura-Benutzerhandbuch.docx"');
            
            // Sende die konvertierte Datei
            res.sendFile(docxPath, async (err) => {
              if (err) {
                console.error('Fehler beim Senden der DOCX-Datei:', err);
              }
              
              // Bereinige temporäre Dateien
              try {
                const { cleanupConversionFiles } = require('./services/document-converter');
                await cleanupConversionFiles(docxPath);
              } catch (cleanupErr) {
                console.warn('Fehler beim Bereinigen der temporären DOCX-Datei:', cleanupErr);
              }
            });
          } catch (convErr) {
            console.error('Fehler bei der DOCX-Konvertierung:', convErr);
            res.status(500).send('Fehler bei der Konvertierung des Benutzerhandbuchs zu DOCX');
          }
        })();
      } catch (err) {
        console.error('Fehler beim Initialisieren der DOCX-Konvertierung:', err);
        res.status(500).send('Fehler beim Konvertieren des Benutzerhandbuchs');
      }
      return;
    }
    ...
    if (format === 'epub') {
      try {
        const { convertMarkdownToEpub } = require('./services/document-converter');
        
        // Asynchroner Prozess für die Konvertierung
        (async () => {
          try {
            // Konvertiere zu ePub
            const epubPath = await convertMarkdownToEpub(
              mdFilePath,
              'Bau-Structura Benutzerhandbuch',
              'Bau-Structura AG'
            );
            
            // Sende die Datei
            res.setHeader('Content-Type', 'application/epub+zip');
            res.setHeader('Content-Disposition', 'attachment; filename="Bau-Structura-Benutzerhandbuch.epub"');
            
            // Sende die konvertierte Datei
            res.sendFile(epubPath, async (err) => {
              if (err) {
                console.error('Fehler beim Senden der ePub-Datei:', err);
              }
              
              // Bereinige temporäre Dateien
              try {
                const { cleanupConversionFiles } = require('./services/document-converter');
                await cleanupConversionFiles(epubPath);
              } catch (cleanupErr) {
                console.warn('Fehler beim Bereinigen der temporären ePub-Datei:', cleanupErr);
              }
            });
          } catch (convErr) {
            console.error('Fehler bei der ePub-Konvertierung:', convErr);
            res.status(500).send('Fehler bei der Konvertierung des Benutzerhandbuchs zu ePub');
          }
        })();
      } catch (err) {
        console.error('Fehler beim Initialisieren der ePub-Konvertierung:', err);
        res.status(500).send('Fehler beim Konvertieren des Benutzerhandbuchs');
      }
      return;
    }
    ...
    // Unbekanntes Format
    res.status(400).send('Unbekanntes Format. Unterstützte Formate: md, docx, epub');
  });
  
  // Zusätzliche Route mit großem B für die Kompatibilität
  app.get('/api/docs/Benutzerhandbuch/download', (req, res) => {
    // Weiterleitung zur kleingeschriebenen Version für Konsistenz
    res.redirect(`/api/docs/benutzerhandbuch/download${req.query.format ? `?format=${req.query.format}` : ''}`);
  });

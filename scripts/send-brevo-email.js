/**
 * Sendet eine E-Mail über den Brevo-API-Dienst
 * 
 * Nutzung:
 * node send-brevo-email.js
 */
...
async function sendWelcomeEmail() {
  ...
  if (!apiKey) {
    console.error('Fehler: BREVO_API_KEY Umgebungsvariable ist nicht gesetzt');
    process.exit(1);
  }
...
Hinweis: Diese E-Mail wurde automatisch am ${heute} generiert. Bitte antworten Sie nicht direkt auf diese Nachricht.
  ...
  try {
    console.log(`\nSende Willkommens-E-Mail an ${recipient} über Brevo API...`);
    
    // Brevo API-Anfrage vorbereiten
    const apiUrl = 'https://api.brevo.com/v3/smtp/email';
    
    const emailData = {
      sender: {
        email: "info@bau-structura.de",
        name: "Bau-Structura"
      },
      to: [
        {
    ...
    if (response.ok) {
      ...
      console.error(`❌ Fehler beim Senden der E-Mail: HTTP Status ${response.status}`);
